function trialMain(params) {
	...
	//var choices_OBE = ['Bet Blue', 'Bet Red', 'Observe'];
	var ps = [0, 0];
	var transitions_ep = [];
	var ps_ep = [];
	//console.log(params.n_episodes_test, n_episodes_train)
	...
	var feedback = {
		...
		stimulus: function(data) {
			...
			// for OBSERVE trials
				...
				// with +1
				// feedback_observe = format_image_button('observe/Slide3')
	...
	var efficacy_cue = {
		...
		stimulus : function() {
			...
			if (eff == 0) {
				...
				//descriptor_machine = 'half the time (entirely unpredictably, so it is impossible to predict when it will happen and time your bets accordingly)';
				...
				//descriptor_machine = 'slightly more than half the time (entirely unpredictably, so it is impossible to predict when it will happen and time your bets accordingly)';
    ...
    function build_trajectory_feedback() {
        ...
        //header = `<p>Episode ` + (i_episode + 1).toString() + `/` + params.n_episodes + `</p><p>Rewards earned this episode: ` + rewards_tally + `</p><p>Total rewards earned: ` + total_rewards_tally + `</p>`;
		if (i_episode < params.n_episodes_train)
		{
    ...
    var feedback_train_episode = {
        ...
        on_finish : function end_ep (data) {
			...
			i_episode++;
			console.log('saving data with group')
			setup();
			//saveData(params, jsPsych.data.getLastTrialData().csv(), temporary = false)
		},
	...
	var train_episode = {
		//timeline: [efficacy_cue, trial, efficacy_survey, feedback_train_episode, save],
		timeline: [efficacy_cue, trial, efficacy_slider, feedback_train_episode, save],
		//timeline: [trial, efficacy_survey, feedback_train_episode, save],
		timeline_variables: params.effs_train,
		randomize_order: true,
		//repetitions: params.n_episodes,
		sample: {
    ...
    var feedback_test_episode = {
        ...
        pages : function() {
            ...
            pages = [
			`<p>On the next screen, you will first be shown a <strong>summary of how many points you earned</strong>, and then on the following one, an <strong>overview of the trajectory</strong> showing you which arm was the right one on a given trial, ` +
                    `whether you chose to observe or bet on a light, and, if you bet, which arm you chose to bet on and which arm you actually ended up betting on.</p>`,
            `<p>Set ` + (i_episode - params.n_episodes_train + 1).toString() + `/` + params.n_episodes_test + `</p><p>Rewards earned this set: ` + rewards_tally + `</p><p>Total rewards earned: ` + total_rewards_tally + `</p>`,
			build_trajectory_feedback(),
            ];
