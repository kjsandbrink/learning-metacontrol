...
save_results_base = os.path.join('data', 'perturbed', 'levc', )
    ...
    for ape_model in ape_models:
        if target_taus is not None:
            for target_tau in target_taus:
                ...
                p = mp.Process(target=sample_perturbed_model_trajectory, args=(ape_model, test_taus, os.path.join(save_results_base, ape_model), True, True, n_repeats_case, '', timestamp, device, os.path.join(model_folder, ape_model), target_tau))
                p.start()
                processes.append(p)
